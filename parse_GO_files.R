...
if(file.exists(annotations_file) & 
   file.exists(reduced_graph_file) & 
   file.exists(reduced_graph_50_file) &
   file.exists(reduced_graph_100_file)){
#  cat(sprintf("GO files already parsed!"))
  ...
  if(!file.exists(reduced_graph_100_file)){
    load(reduced_graph_file)
    gene_sets = G$gene_sets
    to_remove = names(which(sapply(gene_sets, length) > 100))
    G = subset_graph(to_remove, G)
    ids = names(G$C)
    m = length(ids)
    gene_sets = gene_sets[ids]
    G$gene_sets = gene_sets
    G$m = m
    save(G, file = reduced_graph_100_file)
  }
}