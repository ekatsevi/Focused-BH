...
phen_file = sprintf("%s/data/biobank/diagnosed_disease.tab", base_dir)
geno_file = sprintf("%s/data/biobank/HLA.tab", base_dir)
allele_file = sprintf("%s/data/biobank/haplotype_names.txt", base_dir)
...
HES_affected_file = sprintf("%s/data/biobank/HES_affected.Rda", base_dir)
...
pvals_filename = sprintf("%s/data/biobank/pvals_HES_%s_m%d.tsv", base_dir, allele, min_affected)
...
Leaf_BH_rejections = p.adjust(P[num_affected >= min_affected][sapply(G_to_test$C, length) == 0], "fdr") <= q
