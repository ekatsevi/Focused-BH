...
######### RAW DATA FILENAMES ###################
tree_file = "/home/ekatsevi/Documents/large_file_storage/filtered_BH/biobank/coding19.tsv"
phen_file = "/home/ekatsevi/Documents/large_file_storage/filtered_BH/biobank/diagnosed_disease.tab"
...
HES_affected_file = "/home/ekatsevi/Documents/large_file_storage/filtered_BH/biobank/HES_affected.Rda"
...
vertex_labels = NULL
vertex_size = NULL
plot_tree(G, vertex_shapes, frame_colors, fill_colors, vertex_labels, vertex_size)

######### CREATE P-VALUES ###############

pvals_filename = sprintf("pvals_HES_%s.tsv", allele)
if(file.exists(pvals_filename)){
  ...
  for(k in 1:m){
    ...
    if(tree$selectable[k] == "N"){
      P[k] = 1
    } else{
      affecteds = affected[[as.character(tree$coding[k])]]
...
BH_rejections = p.adjust(P, "fdr") <= q*m/sum(tree$selectable == "Y")
sum(BH_rejections)
...
G_anc_sel = subset_graph(ancestors & !not_selectable,G)
...
nodes_to_count = rep(TRUE, G$n)
num_rejections = get_num_rejections(filter_type, base_weights, P, G, nodes_to_count)
